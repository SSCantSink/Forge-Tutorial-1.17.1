...
import com.karanveer.tutorialmod.item.ModArmorMaterial;

import java.util.Map;

public class ModArmorItem extends ArmorItem {

    // This maps a particular armor material to a mobeffect
    private static final Map<ArmorMaterial, MobEffect> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffect>())
                    .put(ModArmorMaterial.TITANIUM, MobEffects.MOVEMENT_SLOWDOWN).build();

    public ModArmorItem(ArmorMaterial material, EquipmentSlot slot, Properties settings) {
        super(material, slot, settings);
    }

    // this happens everytime the player has the item in the inventory
    @Override
    public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
        if(!world.isClientSide()) { // only on server side
            if(entity instanceof Player) { // only to a player
                Player player = (Player)entity;

                if( hasFullSuitOfArmorOn(player))  {
                    evaluateArmorEffects(player); // checks if player has right armor and stuff
                }
            }
        }
